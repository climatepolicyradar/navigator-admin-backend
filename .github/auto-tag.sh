...
if { [ "${is_major}" = true ] && [ "${is_minor}" = true ]; } ||
	...
	echo "Version number without the v: ${version_numbers}"
	version_numbers=${version_numbers%-*} # Remove the trailing '-beta'
	echo "Version number without the beta: ${version_numbers}"
	...
	if [ "${is_minor}" = true ]; then
		...
		new_tag=v${breaking_change_version}.${major_version}.${minor_version+ 1}-beta
		echo "Tagging as new minor version ${new_tag}..."
	elif [ "${is_major}" = true ]; then
		new_tag="v${breaking_change_version}.${major_version+ 1}.0-beta"
		echo "Tagging as new major version ${new_tag}..."
	elif [ "${is_breaking_change}" = true ]; then
		new_tag=v${breaking_change_version+ 1}.0.0-beta
