...
is_minor=false
if [ $(echo "${pull_request_body}" | grep -c "[ ] Minor version") -gt 0 ]; then
	is_minor=true
fi

# Check if major version.
is_major=false
if [ $(echo "${pull_request_body}" | grep -c "[ ] Major version") -gt 0 ]; then
	is_major=true
fi

# Check if breaking change.
is_breaking_change=false
if [ $(echo "${pull_request_body}" | grep -c "[ ] Breaking change") -gt 0 ]; then
	is_breaking_change=true
fi

# If multiple have been checked or none have been checked, don't auto tag.
if [ "${is_major}" = true ] && [ "${is_minor}" = true ] && [ "${is_breaking_change}" = true ]; then
	echo "Can't determine tag information. Auto-tagging failed..."
