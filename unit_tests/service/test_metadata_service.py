...
def test_metadata_fails_when_no_data():
...
def test_metadata_fails_when_invalid_key():
    with pytest.raises(ValidationError) as e:
...
def test_metadata_fails_when_any_and_allow_blanks():
    # This should never fail
    pass


def test_metadata_fails_when_values_and_allow_blanks():
    with pytest.raises(ValidationError) as e:
        _validate(SCHEMA_VALUES_BLANKS, {"author": "henry"})
    expected_msg = "Value 'henry' is not in the allowed list: ['peter', 'joel', 'mark']"
    assert e.value.message == expected_msg


def test_metadata_fails_any_and_no_blanks():
    with pytest.raises(ValidationError) as e:
...
def test_metadata_fails_values_and_no_blanks():
    with pytest.raises(ValidationError) as e:
