...
def test_update_raises_when_family_organisation_mismatch_with_user_org(
    ...
    with pytest.raises(AuthorisationError) as e:
        ok = family_service.update("a.b.c.d", USER_EMAIL, updated_family)
        assert not ok

    expected_msg = f"User '{USER_EMAIL}' is not authorised to perform operation on 'CCLW.corpus.i00000001.n0000'"
    assert e.value.message == expected_msg
