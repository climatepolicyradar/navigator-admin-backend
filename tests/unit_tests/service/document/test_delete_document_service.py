...
def test_delete(document_repo_mock, admin_user_context):
...
def test_delete_raises_when_invalid_id(document_repo_mock, admin_user_context):
    import_id = "invalid"
...
def test_delete_returns_none_when_missing(document_repo_mock, admin_user_context):
...
def test_delete_raises_when_org_mismatch(
    ...
    with pytest.raises(AuthorisationError) as e:
        ok = doc_service.delete("a.b.c.d", another_admin_user_context)
        assert not ok
...
def test_delete_success_when_org_mismatch_super(document_repo_mock, super_user_context):

    ok = doc_service.delete("a.b.c.d", super_user_context)
    assert ok
    assert document_repo_mock.get.call_count == 1
    assert document_repo_mock.get_org_from_import_id.call_count == 1
    assert document_repo_mock.delete.call_count == 1
