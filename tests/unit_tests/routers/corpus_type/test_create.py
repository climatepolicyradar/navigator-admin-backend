...
def test_create_when_ok(
    client: TestClient, corpus_type_service_mock, user_header_token
):
    new_data = create_corpus_type_create_dto("test").model_dump(mode="json")
    response = client.post(
        "/api/v1/corpus-types", json=new_data, headers=user_header_token
    )
