...
def test_get_family_returns_family(family_repo):
    result: FamilyDTO = family_repo.get("sample1")
...
def test_delete_family_when_missing(family_repo):
    ok = family_repo.delete("sample9")
    assert not ok


def test_update_family(family_repo):
    family = family_repo.get("sample1")
    assert family is not None
    family.slug = "snail"

    result = family_repo.update("sample1", family)
    assert result is not None
    assert result.slug == "snail"

    # Also re-check getting the family again
    new_family = family_repo.get("sample1")
...
def test_update_family_when_missing(family_repo):
    family = family_repo.get("sample1")
    assert family is not None
    new_family = family_repo.update("sample9", family)
...
def test_create_family(family_repo):
    ...
    family = family_repo.create(new_family)
    assert family is not None

    # Also re-check getting the family again
    new_family = family_repo.get("test")
...
def test_create_family_when_preexisting(family_repo):
    ...
    result = family_repo.create(new_family)
    assert result is None
